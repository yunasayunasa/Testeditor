...
3. **Undo/Redo機能の不具合**
   - ボタンは表示されているが機能していない
   - コマンド記録のタイミングやイベントフックに問題の可能性

4. **アニメーションタブが反応しない**
   - タブ切り替え機能が未実装またはイベントリスナーの不具合

5. **GizmoManagerの重複**
   - `src/editor/GizmoManager.js`と`src/plugins/GizmoManager.js`が両方存在
   - 統合が必要か確認が必要（AI判断のため現状維持）

## 🎯 優先順位付きタスクリスト

### 🔴 最優先（P0）- VSLエディタの復旧

#### タスク 1-1: VSLエディタのCSS実装

**目的**: ノードベースのビジュアルスクリプティングエディタを視覚的に表示する
